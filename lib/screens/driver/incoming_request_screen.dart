// PATH: lib/screens/driver/incoming_request_screen.dart
import 'dart:async';
...
class _IncomingRequestScreenState extends State<IncomingRequestScreen>
  ...
  void _initSocket() {
    ...
    socket.initTechnician(
      technicianId: techId,
      token: widget.technicianToken.trim(),
    );
  ...
  void _onOrderAccepted(Map<String, dynamic> order) {
    ...
    final chatId = (order["chatId"] ?? order["_id"])?.toString() ?? "";
    if (chatId.trim().isEmpty) {
      _resetState();
      return;
    }

    if (_sheetOpen && Navigator.of(context, rootNavigator: true).canPop()) {
  ...
  void _startTimeout() {
    ...
    _timeoutTimer = Timer(const Duration(seconds: 25), () {
      ...
      final id = currentRequest?['_id']?.toString();
